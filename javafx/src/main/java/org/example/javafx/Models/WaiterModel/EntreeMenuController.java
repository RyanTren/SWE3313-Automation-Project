...
public class EntreeMenuController implements Initializable {
    ...
    @FXML
    private Button entreeOne;
    @FXML
    private Button entreeTwo;
    @FXML
    private Button entreeThree;
    @FXML
    private Button entreeFour;
    ...
    private enum MENU_ITEM {
        PULLEDPORK("pulled pork", "porkImage", 20.00f),
        SAUSAGES("house-made sausages", "sausageImage", 20.00f),
        BRISKET("prime beef brisket", "brisketImage", 36.00f),
        RIBS("spare ribs", "ribImage", 38.00f);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Drink Button Logic
        entreeOne.setOnAction(this::handleItemClick);
        entreeTwo.setOnAction(this::handleItemClick);
        entreeThree.setOnAction(this::handleItemClick);
        entreeFour.setOnAction(this::handleItemClick);
...
//        });
    ...
    private void handleItemClick(ActionEvent event) {
        ...
        switch (imageView.getId()) {
            case "porkImage":
                addItemToCart(MENU_ITEM.PULLEDPORK);
                updateUI(image, imageView.getId());
                break;
            case "sausageImage":
                addItemToCart(MENU_ITEM.SAUSAGES);
                updateUI(image, imageView.getId());
                break;
            case "brisketImage":
                addItemToCart(MENU_ITEM.BRISKET);
                updateUI(image, imageView.getId());
                break;
            case "ribImage":
                addItemToCart(MENU_ITEM.RIBS);
                updateUI(image, imageView.getId());
                break;
        }
    }
        ...
        if (!cartContainer.getChildren().isEmpty()) {
            for (Node n : cartContainer.getChildren()) {
                ...
                if (itemImageView.getId().equalsIgnoreCase(imageID)) {
                    ...
                    int quantity = (int) Model.getInstance().currentOrder.entrees.get(mui.getName());
    ...
    private void addItemToCart(MENU_ITEM item, String direction) {
        ...
        if (cart.entrees.containsKey(item.getName())) {
            // System.out.println("Item already in order. Updating quantity");
            Integer quantity = (Integer) cart.entrees.get(item.getName());
            if (direction.equalsIgnoreCase("add")) {
                cart.entrees.put(item.getName(), quantity + 1);
            } else {
                if (quantity > 0) {
                    cart.entrees.put(item.getName(), quantity - 1);
                }
            ...
            cart.entrees.put(STR."\{item.getName()}", 1);
        }
