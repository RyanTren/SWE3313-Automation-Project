...
public class StarterMenuController implements Initializable {
    ...
    @FXML
    private Button starterOne;
    @FXML
    private Button starterTwo;
    @FXML
    private Button starterThree;
    @FXML
    private Button starterFour;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        starterOne.setOnAction(this::handleItemClick);
        starterTwo.setOnAction(this::handleItemClick);
        starterThree.setOnAction(this::handleItemClick);
        starterFour.setOnAction(this::handleItemClick);
...
//        });
    ...
    private void handleItemClick(ActionEvent event) {
        ...
        switch (imageView.getId()) {
            case "cornbreadImage":
                addItemToCart(MENU_ITEM.CORNBREAD);
                updateUI(image, imageView.getId());
                break;
            case "loafImage":
                addItemToCart(MENU_ITEM.LOAF);
                updateUI(image, imageView.getId());
                break;
            case "martinImage":
                addItemToCart(MENU_ITEM.POTATOBUN);
                updateUI(image, imageView.getId());
                break;
            case "wingImage":
                addItemToCart(MENU_ITEM.WINGS);
                updateUI(image, imageView.getId());
                break;
        }
    }
        ...
        if (!cartContainer.getChildren().isEmpty()) {
            for (Node n : cartContainer.getChildren()) {
                ...
                if (itemImageView.getId().equalsIgnoreCase(imageID)) {
                    ...
                    int quantity = (int) Model.getInstance().currentOrder.starters.get(mui.getName());
    ...
    private void addItemToCart(MENU_ITEM item, String direction) {
        ...
        if (cart.starters.containsKey(item.getName())) {
            // System.out.println("Item already in order. Updating quantity");
            Integer quantity = (Integer) cart.starters.get(item.getName());
            if (direction.equalsIgnoreCase("add")) {
                cart.starters.put(item.getName(), quantity + 1);
            } else {
                if (quantity > 0) {
                    cart.starters.put(item.getName(), quantity - 1);
                }
            ...
            cart.starters.put(STR."\{item.getName()}", 1);
        }
