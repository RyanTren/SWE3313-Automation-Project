...
public class SidesMenuController implements Initializable {
    ...
    @FXML
    private Button sideOne;
    @FXML
    private Button sideTwo;
    @FXML
    private Button sideThree;
    @FXML
    private Button sideFour;
    ...
    private enum MENU_ITEM {
        COLESLAW("Coleslaw", "coleslawImage", 3.00f),
        POTATOSALAD("Potato Salad", "potatosaladImage", 5.00f),
        MNC("Mac N Cheese", "mncImage", 6.50f),
        COLLARDGREENS("Collard Greens", "collardgreensImage", 3.00f);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Sides Button Logic
        sideOne.setOnAction(this::handleItemClick);
        sideTwo.setOnAction(this::handleItemClick);
        sideThree.setOnAction(this::handleItemClick);
        sideFour.setOnAction(this::handleItemClick);
...
//        });
    ...
    private void handleItemClick(ActionEvent event) {
        ...
        switch (imageView.getId()) {
            case "coleslawImage":
                addItemToCart(MENU_ITEM.COLESLAW);
                updateUI(image, imageView.getId());
                break;
            case "potatosaladImage":
                addItemToCart(MENU_ITEM.POTATOSALAD);
                updateUI(image, imageView.getId());
                break;
            case "mncImage":
                addItemToCart(MENU_ITEM.MNC);
                updateUI(image, imageView.getId());
                break;
            case "collardgreensImage":
                addItemToCart(MENU_ITEM.COLLARDGREENS);
                updateUI(image, imageView.getId());
                break;
        }
    }
        ...
        if (!cartContainer.getChildren().isEmpty()) {
            for (Node n : cartContainer.getChildren()) {
                ...
                if (itemImageView.getId().equalsIgnoreCase(imageID)) {
                    ...
                    int quantity = (int) Model.getInstance().currentOrder.sides.get(mui.getName());
    ...
    private void addItemToCart(MENU_ITEM item, String direction) {
        ...
        if (cart.sides.containsKey(item.getName())) {
            // System.out.println("Item already in order. Updating quantity");
            Integer quantity = (Integer) cart.sides.get(item.getName());
            if (direction.equalsIgnoreCase("add")) {
                cart.sides.put(item.getName(), quantity + 1);
            } else {
                if (quantity > 0) {
                    cart.sides.put(item.getName(), quantity - 1);
                }
            ...
            cart.sides.put(STR."\{item.getName()}", 1);
        }
